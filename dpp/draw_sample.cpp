//
void draw_sample(string data_str, int entry=0, int n1=0, int n2=1000)
{
  ...
  tr->SetBranchAddress(TString::Format("%s",data_str.c_str()).Data(), &data);
  tr->GetEntry(entry);
  ...
  gr_data->SetTitle(data_str.c_str());
  gr_data->Draw("AL");
